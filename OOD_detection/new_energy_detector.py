...
def load_dataset(dataset_name, data_dir, train=True, model_type=None):
    ...
    if dataset_name.lower() == 'cifar10':
        ...
        num_classes = 200
         ...
         if not train:  # Use test set for OOD
            try:
                # The correct split parameter should be 'test', not 'val'
                ...
                # Fallback to the validation split if test is not available
                dataset = torchvision.datasets.DTD(
                    root=data_dir, 
                    split='val',  # Use validation split as fallback
                    download=True, 
                    transform=transform
                )
            num_classes = 47
        ...
        if not train:  # Only use test set for OOD
            torchvision.datasets.LSUN._verify_classes = lambda self, classes: list(classes)
            # LSUN_data = torchvision.datasets.LSUN(root=data_dir, classes=["bedroom_val", "classroom_val"])
            dataset = torchvision.datasets.LSUN(root=data_dir, classes=['bedroom_val','classrom_val'], transform=transform)
            num_classes = 10
        else:
